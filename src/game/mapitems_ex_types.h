// This file can be included several times.
UUID(MAPITEMTYPE_TEST, "mapitemtype-test@ddnet.tw")
UUID(MAPITEMTYPE_AUTOMAPPER_CONFIG, "mapitemtype-automapper-config@ddnet.tw")
UUID(MAPITEMTYPE_ENVPOINTS_BEZIER, "mapitemtype-envpoints-bezier@ddnet.tw")
UUID(MAPITEMTYPE_FOLDER_NODE, "mapitemtype-folder-node@ddnet.tw")
UUID(MAPITEMTYPE_LAYER_GROUP_NODE, "mapitemtype-layer-group-node@ddnet.tw")
// UUID(MAPITEMTYPE_OBJECT, "mapitemtype-object@ddnet.tw")
UUID(MAPITEMTYPE_TREENODE, "mapitemtype-treenode@ddnet.tw")
