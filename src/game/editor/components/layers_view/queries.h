#ifndef GAME_EDITOR_COMPONENTS_LAYERS_VIEW_NODE_REFS_H
#define GAME_EDITOR_COMPONENTS_LAYERS_VIEW_NODE_REFS_H

#include "node.h"
#include "nodes.h"

#include <base/hash_combine.h>
#include <base/tuple.h>

#include <game/client/ui_treeview.h>
#include <memory>
#include <tuple>
#include <type_traits>

struct CNodeQueryHash
{
	template<typename... Keys, class = std::enable_if_t<(sizeof...(Keys) > 0)>>
	CNodeQueryHash(Keys &&...QueryKeys)
	{
		constexpr auto Hasher = CTupleHasher();
		const auto Tuple = std::make_tuple<Keys...>(std::forward<Keys>(QueryKeys)...);
		m_Hash = 0;
	}

	size_t Hash() const { return m_Hash; }

	static CNodeQueryHash Combine(const size_t &TypeHash, const CNodeQueryHash &QueryHash)
	{
		size_t Seed = TypeHash;
		HashCombine(Seed, QueryHash.Hash());
		return CNodeQueryHash{};
	}

private:
	CNodeQueryHash() :
		m_Hash(0) {}

	size_t m_Hash;

public:
	struct Hasher
	{
		size_t operator()(const CNodeQueryHash &QueryHash) const { return QueryHash.Hash(); }
	};
};

struct CLayerNodeQuery
{
	int m_GroupIndex;
	int m_LayerIndex;

	explicit CLayerNodeQuery(const int GroupIndex, const int LayerIndex) :
		m_GroupIndex(GroupIndex), m_LayerIndex(LayerIndex) {}

	bool operator()(const std::shared_ptr<ITreeNode> &pNode) const
	{
		if(const auto pLayerNode = GetLayerNode(pNode))
			return pLayerNode->m_Index == m_LayerIndex && pLayerNode->m_GroupIndex == m_GroupIndex;

		return false;
	}

	static std::shared_ptr<CLayerNode> GetLayerNode(const std::shared_ptr<ITreeNode> &pNode)
	{
		if(pNode->Type() == ITreeNode::TYPE_LAYER || pNode->Type() == ITreeNode::TYPE_ENTITIES_LAYER)
			return std::static_pointer_cast<CLayerNode>(pNode);
		return nullptr;
	}

	auto Hash() const { return CNodeQueryHash(m_GroupIndex, m_LayerIndex); }
};

struct CLayerGroupNodeQuery
{
	int m_GroupIndex;

	explicit CLayerGroupNodeQuery(const int GroupIndex) :
		m_GroupIndex(GroupIndex) {}

	bool operator()(const std::shared_ptr<ITreeNode> &pNode) const
	{
		if(pNode->Type() != ITreeNode::TYPE_LAYER_GROUP)
			return false;

		const std::shared_ptr<CLayerGroupNode> &pLayerGroupNode = std::static_pointer_cast<CLayerGroupNode>(pNode);
		return pLayerGroupNode->m_GroupIndex == m_GroupIndex;
	}
};

struct CLayerGroupNodeChildrenQuery
{
	int m_GroupIndex;

	explicit CLayerGroupNodeChildrenQuery(const int GroupIndex) :
		m_GroupIndex(GroupIndex) {}

	bool operator()(const std::shared_ptr<ITreeNode> &pNode) const
	{
		if(const auto pLayerNode = CLayerNodeQuery::GetLayerNode(pNode))
			return pLayerNode->m_GroupIndex == m_GroupIndex;

		return false;
	}

	auto Hash() const { return CNodeQueryHash(m_GroupIndex); }
};

struct CNodeTypeQuery
{
	int m_Type;

	explicit CNodeTypeQuery(const int Type) :
		m_Type(Type) {}

	bool operator()(const CTreeNodePath &Path, const std::shared_ptr<ITreeNode> &pNode) const
	{
		return pNode->Type() == m_Type;
	}
};

#endif
